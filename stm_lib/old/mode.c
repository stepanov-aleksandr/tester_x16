...
void chek_chanall_mode_open(uint16_t chanall)
{
	...
	while(counter_chanall < MAX_CHANAL)
	{
		...
		{
			array_chek[counter_array_chek][counter_chanall] = GPIO_ReadInputDataBit(PORT_CHANAL[counter_chanall],CHANAL[counter_chanall]);
		}
	...
	if(type_test_signal == 1)
	{
		for(counter_chanall = 0;counter_chanall < NUMB_ADC; counter_chanall++)
		{
			if(counter_chanall != chanall)
			{
				...
				if(adc_cyr < adc_cal) adc_cyr = adc_cal;
...
void show_result_work_mode_short()
{
	...
	#ifdef OUT_UART
	send_Uart_str(USART3,"Result Switch "); Exponentiation
	send_Uart_str(USART3,"check = ");
...
void mode_matrix(void)
{
	...
	while(GPIO_ReadInputDataBit(PORT_CAP,CAP_MODE) == 0)
	{
		...
		for(j = 0; j < MAX_CHANAL; j++)
		{
			...
			{
				lcd_clear();
				if(((j % 2) > 0) || (j == 0) )lcd_set_xy(0,0);
				if((j % 2) == 0) lcd_set_xy(0,1);
				lcd_out_char("SW");
				lcd_out_number(j);
				lcd_out_char(" ");
				...
				if(array_chek[i][j] ==1)
				{
					if((j % 2) > 0 )lcd_set_xy(13,0);
					if((j % 2) == 0 )lcd_set_xy(13,1);
					lcd_out_char("ON");
				}
				else
				{
...
uint16_t adc_calib(void)
{
	...
	for(ADC_call_buffer_counter = 0; ADC_call_buffer_counter < ADC_CAL_CIKL;ADC_call_buffer_counter++)
	{
		cyr_ADC_value = get_adc_value();
		if (cal < cyr_ADC_value ) cal = cyr_ADC_value;
	}
...
uint16_t ADC_CH_control(uint16_t ADC_chanall,FunctionalState ADC_CH_STATE)
{
		...
		ADC_RegularChannelConfig(ADC1, ADC_CHANALL[ADC_chanall], 1, ADC_SampleTime_1Cycles5);
...
void adjust()
{
	...
	while(push_mode == 0)
	{
		...
		while(push_up_down == 0 && push_mode == 0)
		{
			...
			if(push_enter == 1)
			{
				if(number_repetitions_regime_open == 0) number_repetitions_regime_open = 1;
					else
		...
		while(push_up_down == 0 && push_mode == 0)
		{
			...
			lcd_set_xy(0,1);
			if(push_enter == 1) matrix_for_validation = ~matrix_for_validation;
			if(matrix_for_validation == 0)
				lcd_out_char("infinitely");
				else lcd_out_char("Once");
			while(1)
			{
				push_enter = GPIO_ReadInputDataBit(PORT_CAP,CAP_ENTER);
				push_up_down = GPIO_ReadInputDataBit(PORT_CAP,CAP_UP_DOWN);
		...
		while(push_up_down == 0 && push_mode == 0)
		{
			...
			lcd_set_xy(0,1);
			if(push_enter == 1) number_of_channels = ++number_of_channels;
			if(number_of_channels == MAX_CHANAL) number_of_channels = 0;
		...
		while(push_up_down == 0 && push_mode == 0)
		{
			...
			lcd_set_xy(0,1);
			if(push_enter == 1) pin_null = ++pin_null;
			if(pin_null == MAX_CHANAL) pin_null = 0;
		...
		while(push_up_down == 0 && push_mode == 0)
		{
			...
			lcd_set_xy(0,1);
			if(push_enter == 1) numb_bits = ++numb_bits;
			if(numb_bits == MAX_CHANAL) numb_bits = 0;
		...
		while(push_up_down == 0 && push_mode == 0)
		{
			...
			lcd_set_xy(0,1);
			if(push_enter == 1) adc_level_add = adc_level_add + 50;
			if(adc_level_add == 2000) adc_level_add = 0;
		...
		while(push_up_down == 0 && push_mode == 0)
		{
			...
			lcd_set_xy(0,1);
			if(push_enter == 1) set_option_push = ++set_option_push;
			if(numb_bits == 3) set_option_push = 0;
